...
\title{Comparação de performance entre Plataformas ARM's na execução de um Algoritmo Genético}
...
\begin{abstract}
Routing planning is becoming a daily task for robots, they can be present from the planning of the movement of a robotic manipulator to a route of cleaning a domestic robot, requiring that the routes, previously planned by external computers , to be planned by the robot itself. This paper proposes the execution of a genetic algorithm for route planning based on the symmetric traveling salesman problem in ARM platforms, in which the algorithm will be tested on 4 platforms and measured the performance of the same.
\end{abstract}
     
\begin{resumo} 
O planejamento de rotas está se tornando a cada dia uma tarefa corriqueira para robôs, elas podem estar presentes desde o planejamento do movimento de um manipulador robótico à uma rota de limpeza de um robô doméstico, exigindo que o as rotas, antes planejadas por computadores externos, passem a ser planejadas pelo próprio robô. Este artigo propõe a execução de um algoritmo genético para o planejamento de rotas baseadas no problema do caixeiro viajante simétrico em plataformas ARM's, no qual o algoritmo será testado em 4 plataformas e medido o desemenho das mesmas.
\end{resumo}
...
O problema de roteirização de robôs pode ser modelado através do Problema do Caixeiro Viajante(PCV). Este trabalho tem como objetivo aplicar um algoritmo genético em 4 plataformas embarcadas, as plataformas testadas são \textit{ARM's SBCs} (\textit{single-board computers} ou computadores de placa única), Raspberry pyA+, Raspberry py3, Orange Pi One e BeagleBone Black. 

Na próxima seção será descrito o PCV e sua variação síncrona. Na seção 3 será abordado a fundamentação teórica do problema e da solução a ser implementada e as plataformas embarcadas utilizadas e suas principais características. A seção 4 abordará os métodos de implementação e suas metodologias. Na seção 5  será exposto os resultados obtidos oriundos da seção 4, enquanto a conclusão será apresentada na seção 6, e por fim na seção 7 os trabalhos futuros.
...
\subsection{Formulação Matemática Para o PCV}
...
\subsection{Problema do Caixeiro Viajante Simétrico} \label{sec:PCVS}
...
\end{equation}
onde $J(j)$ é o conjunto de todos os arcos, não direcionados, ligados ao nó $j$ e $E(S)$ é o subconjunto de todos os arcos que ligam as cidades em qualquer conjunto S não vazio que contenha todas as cidades\cite{Wikipedia:17}.
