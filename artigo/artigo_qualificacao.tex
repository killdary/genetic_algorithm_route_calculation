...
\section{Resultados}
	...
	O algoritmo foi executado um total de 30 vezes sendo que a menor distância encontrada foi de 37,38 metros e a pontuação máxima de 125 pontos.
	
	\subsection{Cenário 2 - Rota para 20 pontos, com limitação de custo}	
	O cenário 2 é estabelecido sobre o mesmo conjunto de pontos estabelecidos no cenário 1(figura \ref{fig:Pontos1}), porém na seguinte execução foi imposto a restrição de custo máximo de 23 metros.
	...
	O algoritmo foi executado o mesmo número de vezes que o cenário 1, 30 execuções, sendo que a menor distância encontrada foi de 22,23 metros com uma pontuação de 58,0 pontos. 
	
	
	\subsection{Cenário 3 - Rota para 30 pontos, sem restrições de custo}	
	
	O cenário 3  relacionado com a simulação do cálculo de rota, onde o agente não possui restrições de custo, e pode percorrer todas as cidades livremente. Para este cenário foi utilizado um conjunto de 30 pontos a serem visitados, como mostrado a seguir na figura \ref{fig:Pontos3}.
	\begin{figure}[ht]
		\centering
	...
	O algoritmo foi executado um total de 30 vezes sendo que a menor distância encontrada foi de 71,24 metros e a pontuação máxima de 149 pontos.
	
	\subsection{Cenário 4 - Rota para 30 pontos, com limitação de custo}	
	O cenário 4 é estabelecido sobre o mesmo conjunto de pontos estabelecidos no cenário 3(figura \ref{fig:Pontos3}), porém na seguinte execução foi imposto a restrição de custo máximo de 35 metros.
	...
	O algoritmo foi executado o mesmo número de vezes que o cenário 3, 30 execuções, sendo que a menor distância encontrada foi de  33,66 metros com uma pontuação de 71,0 pontos. 
...
Como proposta futura, tem-se a intenção de adaptar o algoritmo para utilizar rotas presentes no TSPLIB\cite{TSPLIB}, e alterar possíveis formas de mutação e \textit{crossover}, e e adicionar o calculo de rotas com restrições para múltiplos agentes.
%
