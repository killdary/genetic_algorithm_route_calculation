...
class GA_TSPKP:
    ...
    def select_deposit_nearly(self, point):
        """
        metodo que retorna o deposito mais proximo do ponto informado
        :param point: ultimo ponto antes do depósito
        :return: deposito mais proximo
        """
        minor = self.deposits

        for i in self.deposits:
            if self.distancias[point,i] < self.distancias[point, minor]:
                minor = i
...
#        remove points start, end and deposits for vertices avaliables
        
        remove_points = list()

        remove_points = self.start_point + self.end_point


        self.initial_cromossome = np.delete(self.initial_cromossome, remove_points)
